...
function getCustomMenu({ collectionData }) {
  const menuPages = collectionData.filter(post => (post.type === BLOG.NOTION_PROPERTY_NAME.type_menu || post.type === BLOG.NOTION_PROPERTY_NAME.type_sub_menu) && post.status === 'Published').sort(compare('id', true))
  const menus = []
